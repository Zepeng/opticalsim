...
// LBNE10ktOpNewFlashFinder.h

#ifndef LBNE10ktOpNewFlashFinder_H
#define LBNE10ktOpNewFlashFinder_H 1
...
namespace opdet {
  ...
  class LBNE10ktOpNewFlashFinder : public art::EDProducer{
  public:
 
    // Standard constructor and destructor for an ART module.
    LBNE10ktOpNewFlashFinder(const fhicl::ParameterSet&);
    virtual ~LBNE10ktOpNewFlashFinder();
...
#endif // LBNE10ktOpNewFlashFinder_H




// LBNE10ktOpNewFlashFinder_module.cc

// This is a short program required by the ART framework.  It enables
// a program (LBNE10ktOpNewFlashFinder, in this case) to be called as a module
...
namespace opdet {
  DEFINE_ART_MODULE(LBNE10ktOpNewFlashFinder);
}


// LBNE10ktOpNewFlashFinder.cxx
...
namespace opdet {
  ...
  //-----------------------------------------------------------------------
  // Constructor
  LBNE10ktOpNewFlashFinder::LBNE10ktOpNewFlashFinder(fhicl::ParameterSet const& pset)
  {
  ...
  //-----------------------------------------------------------------------
  // Destructor
  LBNE10ktOpNewFlashFinder::~LBNE10ktOpNewFlashFinder() 
  {}
   
  //-----------------------------------------------------------------------
  void LBNE10ktOpNewFlashFinder::beginJob()
  {
  }
   

  //-----------------------------------------------------------------------
  void LBNE10ktOpNewFlashFinder::produce(art::Event& evt) 
  {
    ...
    for(unsigned int i = 0; i < WaveformHandle->size(); ++i)
    {
     ...
     //    mf::LogInfo("LBNE10ktOpNewFlashFinder")<<"Searching for isolated flashes";
    ...
    //   mf::LogInfo("LBNE10ktOpNewFlashFinder")<<"Entering loop"<<std::endl;
		...
		if((HitClaimed[OpChannelID][HitTime]==true))
		  {
		    ...
		    if(FlashID!=PreviousClaimant)
			 ...
			 //		 mf::LogInfo("LBNE10ktOpNewFlashFinder")<<"Combination made, escaping loop " <<std::endl;
