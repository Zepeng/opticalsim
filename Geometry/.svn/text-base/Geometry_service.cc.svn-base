...
namespace geo {
  ...
  void Geometry::InitializeChannelMap()
  {
    ...
    case geo::kArgoNeuT   :
    case geo::kLArIAT	  :
      fChannelMapAlg = new geo::ChannelMapArgoNeuTAlg();
      break;
    case geo::kBo         :
      fChannelMapAlg = new geo::ChannelMapBoAlg();
      break;
    case geo::kLBNE10kt   : 
    case geo::kLBNE34kt   : 
      ...
      break;
    case geo::kLBNE35t    : 
      fChannelMapAlg = new geo::ChannelMap35Alg();
      break;
    default               : 
  ...
  const TPCGeo& Geometry::PositionToTPC(double const  worldLoc[3],
					...
					unsigned int &cstat) const
  {
    return this->PositionToCryostat(worldLoc,cstat).PositionToTPC(worldLoc,tpc);
  }
						  ...
						  unsigned int &cstat) const
  {
    ...
    cstat = UINT_MAX;
    for(unsigned int c = 0; c < this->Ncryostats(); ++c){
      if(worldLoc[0] >= cstatBoundaries[0+c*6] &&
	 worldLoc[0] <= cstatBoundaries[1+c*6] && 
	 worldLoc[1] >= cstatBoundaries[2+c*6] && 
	 worldLoc[1] <= cstatBoundaries[3+c*6] && 
	 worldLoc[2] >= cstatBoundaries[4+c*6] && 
	 worldLoc[2] <= cstatBoundaries[5+c*6] ){
	cstat = c;
	break;
      }
    }
