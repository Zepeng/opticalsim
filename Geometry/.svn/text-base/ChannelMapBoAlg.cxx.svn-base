...
#include "Geometry/ChannelMapBoAlg.h"
...
namespace geo{
  ...
  //----------------------------------------------------------------------------
  // Define sort order for cryostats in bo configuration
  static bool sortCryoBo(const CryostatGeo* c1, const CryostatGeo* c2)
  {
  ...
  //----------------------------------------------------------------------------
  // Define sort order for tpcs in bo configuration.
  static bool sortTPCBo(const TPCGeo* t1, const TPCGeo* t2) 
  {
    double xyz1[3] = {0.};
    double xyz2[3] = {0.};
    double local[3] = {0.};
  ...
  //----------------------------------------------------------------------------
  // Define sort order for planes in bo configuration
  static bool sortPlaneBo(const PlaneGeo* p1, const PlaneGeo* p2) 
  {
    double xyz1[3] = {0.};
    double xyz2[3] = {0.};
    double local[3] = {0.};
  ...
  //----------------------------------------------------------------------------
  bool sortWireBo(WireGeo* w1, WireGeo* w2){
    double xyz1[3] = {0.};
    double xyz2[3] = {0.};

    w1->GetCenter(xyz1); w2->GetCenter(xyz2);
    
    /// Change sorting for wires on plane 2 (x<-7 mm)
    /// plane pitch = 6.5024 mm
    /// plane 0 is at x = 0, plane 1 is at x = -6.5024 mm, plane 2 is at x = -13.0048 mm
    /// Probably not the best way to do it
    if (xyz1[0]>-0.7){//plane 0 and plane 1
  ...
  //----------------------------------------------------------------------------
  ChannelMapBoAlg::ChannelMapBoAlg()
  {
  }
  ...
  //----------------------------------------------------------------------------
  void ChannelMapBoAlg::Initialize(std::vector<geo::CryostatGeo*> & cgeo)
  {
    fNcryostat = cgeo.size();
    
    mf::LogInfo("ChannelMapBoAlg") << "Initializing...";
  

    std::sort(cgeo.begin(), cgeo.end(), sortCryoBo);
    for(size_t c = 0; c < cgeo.size(); ++c) 
      cgeo[c]->SortSubVolumes(sortTPCBo, sortPlaneBo, sortWireBo);
    ...
    LOG_DEBUG("ChannelMapBo") << "# of channels is " << fNchannels;
  ...
  //----------------------------------------------------------------------------
  void ChannelMapBoAlg::Uninitialize()
  {
  }

  //----------------------------------------------------------------------------
  std::vector<geo::WireID> ChannelMapBoAlg::ChannelToWire(uint32_t channel)  const
  {
  ...
  //----------------------------------------------------------------------------
  uint32_t ChannelMapBoAlg::Nchannels() const
  {
    return fNchannels;
  }
  
  //----------------------------------------------------------------------------
  WireID ChannelMapBoAlg::NearestWireID(const TVector3& worldPos,
  ...
  //                    Wire2     v 7
  //
  uint32_t ChannelMapBoAlg::PlaneWireToChannel(unsigned int plane,
					       unsigned int wire,
					       unsigned int tpc,
					       unsigned int cstat) const
  {
    ...
    if(tpc   < fNTPC[cstat] &&
      ...
      throw cet::exception("ChannelMapBoAlg") << "NO CHANNEL FOUND for cryostat,tpc,plane,wire: "
					      << cstat << "," << tpc << "," << plane << "," << wire;
    }
    
    // made it here, that shouldn't happen, return UINT_MAX
    mf::LogWarning("ChannelMapBoAlg") << "should not be at the point in the function, returning "
				      << "UINT_MAX";
    return UINT_MAX;
  ...
  //----------------------------------------------------------------------------
  const SigType_t ChannelMapBoAlg::SignalType(uint32_t const channel) const
  {

    SigType_t sigt;

    if(       channel <  fFirstChannelInThisPlane[0][0][2]     ){ sigt = kInduction;  }
    else if( (channel >= fFirstChannelInThisPlane[0][0][2]) &&
             (channel <  fFirstChannelInNextPlane[0][0][2])    ){ sigt = kCollection; }
  ...
  //----------------------------------------------------------------------------
  const View_t ChannelMapBoAlg::View(uint32_t const channel) const
  {
