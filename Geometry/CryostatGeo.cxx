...
namespace geo{
  ...
  void CryostatGeo::SortSubVolumes( bool (*psortTPCs)   ( const TPCGeo *t1,   const TPCGeo *t2),
                                    bool (*psortPlanes) ( const PlaneGeo *p1, const PlaneGeo *p2),
                                    bool (*psortWires)  (       WireGeo *w1,        WireGeo *w2)     )
  {
    std::sort(fTPCs.begin(), fTPCs.end(), (*psortTPCs));
    for(size_t t = 0; t < fTPCs.size(); ++t){ 
      ...
      fTPCs[t]->SortSubVolumes( (*psortPlanes), (*psortWires) );
    }
