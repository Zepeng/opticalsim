...
// LArSoftification: Eric Church, Yale
// mail:        mmszydagis@ucdavis.edu, matthew.szydagis@gmail.com,
//              echurch@fnal.gov
//
////////////////////////////////////////////////////////////////////////
#include <iostream>
#include "Geant4/G4ParticleTypes.hh" //lets you refer to G4OpticalPhoton, etc.
#include "Geant4/G4EmProcessSubType.hh" //lets you call this process Scintillation
#include "Geant4/G4Version.hh" //tells you what Geant4 version you are running

#include "LArG4/NestS1Light.hh"
...
NestS1Light::NestS1Light(const G4String& processName,
		           G4ProcessType type)
  : 	flni(getLArNest()) 	// grab reference to the singleton, make it a class member.
//  : OpFastScintillation(processName, type) : G4VRestDiscreteProcess(processName, type)
...
NestS1Light::~NestS1Light(){} //destructor needed to avoid linker error

G4VParticleChange*
NestS1Light::AtRestDoIt(const G4Track& aTrack, const G4Step& aStep)

// This routine simply calls the equivalent PostStepDoIt since all the
// necessary information resides in aStep.GetTotalEnergyDeposit()
{
  return NestS1Light::PostStepDoIt(aTrack, aStep);
}

G4VParticleChange*
NestS1Light::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
  ...
  */
  ...
  flni.SetG4Stepper(tmp);
	...
	if ( !WIN && !TOP && !ANE && !SRF && !GAT && !CTH && !BOT && !PMT ) {
	  ElectricField = larp->Efield(); //aMaterialPropertiesTable->
	...
	// EC, add:
	flni.SetEnergyDep(TotalEnergyDeposit);
	flni.SetNumIonEl(NumElectrons);
	flni.SetNumSciPhi(NumPhotons);
	TLorentzVector tmp2(TVector3(x1[0],x1[1],x1[2]), (double)t0 );
	flni.SetLast4Location( tmp2);

	// Below call to Ben Jones's OpFastScintillation::RecordPhotonsProduced() puts 
	// response on the PMTs, choosing birth times. Access to this method is why we 
	// are inheriting NestS1Light from OpFastScintillation. An OpDetPhotonTable
	// results. See ThePhotons object that LArG4_module pulls out and eventually 
	// puts on the event. One can follow the same idea for paddles. 
	// The electrons, meanwhile, will be "drifted" by LArVoxelReadout.
	RecordPhotonsProduced(aStep,(double)NumPhotons);

	/* EC	
	std::cout << "NestS1Light: NumIonElectrons produced:  " << flni.NumberIonizationElectrons() << std::endl;
	std::cout << "NestS1Light: NumScintPhotons produced:  " << flni.NumberScintillationPhotons() << std::endl;
	std::cout << "\t in NEST at following 4-Location: " << std::endl;
	std::cout << "NestS1Light : " << flni.Last4Location()[0] <<  "," << flni.Last4Location()[1] <<   "," << flni.Last4Location()[2] << "," << flni.Last4Location()[3]  << "." << std::endl;
	*/
	...
	/* EC, don't keep this cumulative sums, per TS
...
// GetMeanFreePath
// ---------------
G4double NestS1Light::GetMeanFreePath(const G4Track&,
                                          G4double ,
                                          G4ForceCondition* condition)
{
        *condition = StronglyForced;
	// what this does is enforce the NestS1Light physics process as always
...
// GetMeanLifeTime
// ---------------
G4double NestS1Light::GetMeanLifeTime(const G4Track&,
                                          G4ForceCondition* condition)
{
        *condition = Forced;
