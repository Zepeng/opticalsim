...
namespace larg4 {
...

  {
	...
	// Set the step size to small value if NEST is chosen, per Matthew Szydagis, "because without delta rays, the yields are wrong.  The ICARUS model that is in LArSoft uses a fudge factor to compensate, but NEST is "purer" -- no fudge factor. "
	
	if (fLgpHandle->NESTOn())
	  {
	    G4double maxStep(0.05 * micrometer);
	    G4UserLimits *fStepLimit = new G4UserLimits(maxStep);
	    larTPCLogical->SetUserLimits(fStepLimit);
	    mf::LogInfo("LArVoxelReadoutGeometry")  << " G4StepSizeLimit: " << maxStep;
	  }
